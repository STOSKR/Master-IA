...
def parsear_argumentos():
    ...
    parser = argparse.ArgumentParser(
        description='Ejecutar algoritmo genético con configuración específica de mutación y cruce'
    )
    ...
    # Parámetros de mutación y cruce
    parser.add_argument('--prob-mutacion', type=float, default=0.35,
                        help='Probabilidad de mutación 0.0-1.0 (default: 0.35)')
    parser.add_argument('--prob-cruce', type=float, default=0.8,
                        help='Probabilidad de cruce 0.0-1.0 (default: 0.8)')
    ...
    parser.add_argument('--horas', type=float, default=None,
                        help='Tiempo límite en HORAS (puede ser decimal: 0.5 = 30min, 2 = 2 horas) (default: 2.0)')
    ...
    parser.add_argument('--output-dir', type=str, default='resultados_mutacion_cruce',
                        help='Directorio para guardar archivos (default: resultados_mutacion_cruce)')
...
def ejecutar_con_config(args):
    ...
    import config
    config.PROBABILIDAD_MUTACION = args.prob_mutacion
    config.PROBABILIDAD_CRUCE = args.prob_cruce
    ...
    print(f"  - Prob. Mutación: {args.prob_mutacion*100:.0f}%")
    print(f"  - Prob. Cruce: {args.prob_cruce*100:.0f}%")
    ...
    if args.guardar_json:
        ...
        datos = {
            ...
            'historial_mejor_gen': resultado.get('historial_mejor_gen', resultado['historial_fitness']),
            'historial_tiempos': resultado['historial_tiempos']
        }
        
        # Crear directorio si no existe
        os.makedirs(os.path.dirname(archivo_json) if os.path.dirname(archivo_json) else '.', exist_ok=True)
    ...
    if args.guardar_grafica:
        ...
        axes[0].plot(resultado['historial_fitness'], linewidth=2, color='steelblue', label='Mejor global')
        if 'historial_mejor_gen' in resultado:
            axes[0].plot(resultado['historial_mejor_gen'], linewidth=1.5, color='orange', 
                        alpha=0.7, label='Mejor por generación')
