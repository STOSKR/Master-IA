...
# Set HuggingFace token (required for P1 notebook)
# Make sure to set HF_TOKEN before running: export HF_TOKEN="your_token_here"
if [ -z "$HF_TOKEN" ]; then
    echo "⚠ WARNING: HF_TOKEN environment variable not set"
    echo "   The notebook may fail if HuggingFace authentication is required"
    echo "   To set it, run: export HF_TOKEN='your_token_here' before sbatch"
fi
...
notebook_path = "P1_NLLB_RU-ZH.ipynb"
output_path = f"P1_NLLB_RU-ZH_output_{datetime.now().strftime('%Y%m%d_%H%M%S')}.ipynb"
...
try:
    ...
    error_path = f"P1_NLLB_RU-ZH_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.ipynb"
